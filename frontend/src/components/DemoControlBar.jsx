...
import { Play, Pause, RotateCcw, Zap, ChevronUp, ChevronDown, Check, AlertTriangle } from 'lucide-react';
...
const DemoControlBar = () => {
  ...
  return (
    <>
      {/* COLLAPSED: small floating pill button at bottom-right */}
      {isMinimized && (
        <button
          onClick={() => setIsMinimized(false)}
          className="fixed bottom-4 right-4 z-40 flex items-center gap-2 bg-navy-900/95 dark:bg-navy-950/98 border border-navy-700 text-white shadow-2xl rounded-full px-4 py-2 cursor-pointer select-none transition-all duration-300 hover:bg-navy-800 hover:border-aqua-500/60 group no-print"
          title="Open Simulation Control Center"
        >
          ...
          <span className="text-[11px] font-semibold uppercase tracking-wider text-aqua-400 whitespace-nowrap">
            Demo Panel
          </span>
          <ChevronUp className="w-3.5 h-3.5 text-navy-400 group-hover:text-white transition-colors" />
        </button>
      )}

      {/* EXPANDED: full panel anchored at bottom */}
      {!isMinimized && (
        <div className="fixed bottom-0 right-0 z-40 bg-navy-900/95 dark:bg-navy-950/98 border-t border-navy-800 text-white w-full md:w-[720px] shadow-2xl rounded-t-3xl max-h-[90vh] overflow-y-auto no-print">

          {/* Title / Collapse Bar */}
          <div
            onClick={() => setIsMinimized(true)}
            className="h-11 flex items-center justify-between cursor-pointer border-b border-navy-800/40 px-6 font-sans select-none"
          >
          ...
          {/* Panel Content */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-5 pb-5 px-6">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full text-xs bg-navy-800 border border-navy-700 rounded-lg px-2 py-1.5 text-white focus:outline-none focus:ring-1 focus:ring-aqua-500 cursor-pointer"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-xs bg-navy-800 border border-navy-700 rounded-lg px-2 py-1.5 text-white font-mono focus:outline-none focus:ring-1 focus:ring-aqua-500"
                    />
                  </div>
                </div>
